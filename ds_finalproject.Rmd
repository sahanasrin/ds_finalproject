...
#The first vizualization titled "Number of People in the United States Living in Tracts with Specified Income Brackets" shows us that the number of people living in a middle class census tract is the highest, with the lower middle class bracket following behind. Bearing in mind that this does not give insight into the actual number of people in each of these brackets, it tells us that the majority of Americans are occupying what can be known as "middle-class America." In terms of a specific number of census tracts in each bracket, [FILL IN THIS AFTER YOU FIX THE CODE]
...
states_2017_EDA %>%
  ...
  labs(
    ...
    x = "Income Bracket", 
    y = "Number of People", 
    caption = "ACS 5 Year Survey and Childhood Index Opportunity Scores"
  )
...
#   summarize(
#   poor == sum(poor == 1), 
#   lower-middle == sum(lower_middle == 1), 
#   middle == sum(middle == 1), 
#   upper == sum(upper == 1)
#  )
...
#p2 <- states_2017 %>%
  ...
  #labs(x = "Average Seconds Traveled to Work",
       #y = "Count",
...
pairs(data = states_2017,
    ...
    #Citation for correlation grid: http://www.sthda.com/english/wiki/scatter-plot-matrices-r-base-graphs
```
...
url <- str_glue("https://api.census.gov/data/2017/acs/acs5?get=NAME,B28010_001E,B15003_022E,B08133_001E,B15003_017E&for=state:*")
popurl <- GET(url = url)
http_status(popurl)
popurl <- content(popurl, as = "text")
pop_matrix <- fromJSON(popurl)
...
# url1 <- str_glue("https://api.census.gov/data/2017/acs/acs5?get=NAME,B19013_001E&for=state:*")
# popurl1 <- GET(url = url1)
# http_status(popurl1)
# popurl1 <- content(popurl1, as = "text")
# pop_matrix1 <- fromJSON(popurl1)
# #turn body of character matrix into a tibble
# states1 <- as_tibble(pop_matrix1[1:nrow(pop_matrix1),],.name_repair = "minimal")
# names(states1) <- states1[1,]
# states1 = states1[-1,]
# 
# #take out na values
...
# #only contains values 
# drop <- c("NAME", "state")
# states_numeric1 <- states1[,!(names(states1) %in% drop)]
# states_numeric1[1] <- sapply(states_numeric1[1], as.numeric)
# head(states_numeric1)
# 
# #run PCA on states_numeric
# states_pca1 <- prcomp(states_numeric1, center = TRUE, scale. = TRUE)
# 
# #how much variance explained? 
# summary(states_pca1)
# 
# #extract principle components
# states_pcs1 <- states_pca1 %>% 
#   .$x %>% 
#   as_tibble()
# 
# #combine pc's to names
# states_pcs1 <- bind_cols(
#   select(states1, -state), 
#   select(states_pcs1, PC1, PC2)
# )
# 
# #total within sum of squares to find ideal number of clusters
# fviz_nbclust(states_numeric1, FUN = kmeans, method = "wss")
# #total silhouette width
# fviz_nbclust(states_numeric1, FUN = kmeans, method = "silhouette")
# #gap statistic
# fviz_nbclust(states_numeric1, FUN = kmeans, method = "gap_stat")
# 
# num_kmeans1 <- function(k, data){
#   #generate k-means clusters
#   kmeans <- kmeans(states_numeric1, 
#   centers = k, 
#   nstart = 100
#   );
# #create dataframe that binds together clusters and PC's
#   states_clusters1 <- bind_cols(
#   select(states1, NAME), 
#   select(data, PC1, PC2), 
#   cluster = kmeans$cluster
#   )
# }
# 
# kmeans1_2 <- num_kmeans1(k = 2, data = states_pcs1) 
# kmeans1_2
# 
# 
# ggplot(
